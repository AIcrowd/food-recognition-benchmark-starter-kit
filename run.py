...
def single_annotation(image_id, number_of_points=10):
    ...
    _result["segmentation"] = single_segmentation(
...
def gather_images(test_images_path):
    images = glob.glob(os.path.join(test_images_path, "*.jpg"))
    return images
...
def gather_image_ids(test_images_path):
    ...
    filenames = [
        os.path.basename(image_path).replace(".jpg", "") for image_path in images
    ]
...
def gather_input_output_path():
    test_images_path = os.getenv("AICROWD_TEST_IMAGES_PATH", False)
    assert (
        test_images_path != False
    ), "Please provide the path to the test images using the environment variable : AICROWD_TEST_IMAGES_PATH"

    predictions_output_path = os.getenv("AICROWD_PREDICTIONS_OUTPUT_PATH", False)
    assert (
        predictions_output_path != False
    ), "Please provide the output path (for writing the predictions.json) using the environment variable : AICROWD_PREDICTIONS_OUTPUT_PATH"
