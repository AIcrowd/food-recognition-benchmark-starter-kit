...
def refine_detections(rois, probs, deltas, window, config):
    ...
    if config.DETECTION_MIN_CONFIDENCE:
        filtered_keep_bool = keep_bool & (class_scores >= config.DETECTION_MIN_CONFIDENCE)

        if torch.nonzero(filtered_keep_bool).shape != []:
            keep = filtered_keep_bool
        else:
            """
            If after the filtration shape no boxes are left,
            then simply use the unfiltered set of predictions irrespective of the
            confidence.
            TODO: Think this through better.
            """
            print("Ignoring FILTER STEP")
            print("*"*200)
...
class MaskRCNN(nn.Module):
    ...
    def load_weights(self, filepath, ignore_layers=[]):
        ...
        if os.path.exists(filepath):
            ...
            pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict and k not in ignore_layers}
